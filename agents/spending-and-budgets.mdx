...
Every payment is checked against the budget before it is approved. Cumulative caps also count money still in flight, so concurrent requests cannot race past a cap together. When a payment is denied, the agent gets a stable machine-readable code and the denial shows up in your activity feed; every code is documented on [Errors and denials](/agents/errors-and-denials).

## Auto top-up, your way

You can create a rule that refills an agent's budget from your balance or Earn when it runs low, but only within a monthly ceiling you define. The agent cannot raise that ceiling.
